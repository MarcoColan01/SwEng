...
public class Briscola {
  ...
  Player establishTurnWinner(@NotNull Card first, @NotNull Card second) {
    if(first.getSuit().equals(getBriscola()) || second.getSuit().equals(getBriscola())){
      if(first.getRank().points() > second.getRank().points()) return players[0];
  ...
  public Player establishGameWinner() {
    if(players[0].compareTo(players[1]) > 0) return players[0];
    else if(players[0].compareTo(players[1]) < 0) return players[1];
    return Player.NULL;
  }
